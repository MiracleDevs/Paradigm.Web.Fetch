...
export class HttpClient
{
    ...
    removeInterceptor(interceptor: IHttpInterceptor): void
    {
        ...
        if (index < 0)
            throw new Error("The interceptor does not belong to the http client and can not be removed.");
    ...
    private getUrl(url: string, queryString?: QueryString): string
    {
        ...
        for (const key in queryString)
        {
            ...
            if (value === null)
                throw new Error(`The query string parameter '${key}' is undefined or an invalid object type.`);
