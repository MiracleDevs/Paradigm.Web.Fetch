...
describe("HttpClient", () => {
    ...
    it("should fail if try to remove an interceptor that wasn't registered", () => {
        const httpClient = new HttpClient();
        expect(() => httpClient.removeInterceptor(new ContentTypeInterceptor("application/json"))).toThrowError(
            "The interceptor does not belong to the http client and can not be removed."
        );
        expect(() => httpClient.removeInterceptor(undefined as any)).toThrowError(
            "The interceptor does not belong to the http client and can not be removed."
        );
        expect(() => httpClient.removeInterceptor(null as any)).toThrowError(
            "The interceptor does not belong to the http client and can not be removed."
        );
    });
    ...
    it("should replace query string parameters", async () => {
        const httpClient = new HttpClient();
        httpClient.setFetcher(new MockFetcher());
        const response = await httpClient.get("https://example.com/api/{clientId}/{isEnabled}/{name}?date={date}", {
            clientId: 1,
            isEnabled: true,
            name: "John",
            date: new Date("2012-12-12T12:00:00.000Z"),
        });
    ...
    it("should fail if the query string parameters are invalid", () => {
        const httpClient = new HttpClient();
        httpClient.setFetcher(new MockFetcher());
        expect(() => httpClient.get("https://example.com/api/{clientId}", { clientId: undefined })).rejects.toThrowError(
            "The query string parameter 'clientId' is undefined or an invalid object type."
        );
        expect(() => httpClient.get("https://example.com/api/{clientId}", { clientId: null })).rejects.toThrowError(
            "The query string parameter 'clientId' is undefined or an invalid object type."
        );
        expect(() => httpClient.get("https://example.com/api/{clientId}", { clientId: {} })).rejects.toThrowError(
            "The query string parameter 'clientId' is undefined or an invalid object type."
        );
    });
